...
# Credenciales desde variables de entorno - CON VALIDACIÓN
...
if not APP_CODE or not APP_KEY:
    ...
    logger.error(f"   NUVEI_APP_CODE_SERVER: {APP_CODE}")
    logger.error(f"   NUVEI_APP_KEY_SERVER: {APP_KEY}")
    raise RuntimeError("Credenciales Nuvei no configuradas en variables de entorno")

logger.info(f"✅ Credenciales cargadas: {APP_CODE[:10]}... en {ENV}")

try:
    client = NuveiClient(APP_CODE, APP_KEY, environment=ENV)
    logger.info("✅ NuveiClient inicializado correctamente")
except Exception as e:
    logger.error(f"❌ Error inicializando NuveiClient: {e}")
    raise
...
def create_payment(req: PaymentCreateRequest):
    ...
    try:
        ...
        logger.info(f"📥 Respuesta Nuvei recibida")
        ...
        logger.info(f"✅ Pago creado exitosamente: intent={intent_id}, order={order_id}")
